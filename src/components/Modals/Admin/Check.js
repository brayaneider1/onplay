...
//Modal para modificiar el estado de la solicitud

export const Check = ({ visible, setVisible, data }) => {
  var dataForm;
    ...
    async () =>
      await fetch(
        ...
        .then((response) => {
          ...
          console.warn('respuestota', response);
        })
        .catch((err) => err),
    {
      enabled: false,
    ...
    >
      ...
      >
        <div className="input-component3 w-11/12">
          ...
          >
            <option key="1" value="SC">Recientes</option>
          </select>
        </div>
